...
1. Add a case to `WindowAction` in [Sources/McMacWindowCore/WindowAction.swift](Sources/McMacWindowCore/WindowAction.swift).
2. Add the geometry in `computeTargetRect`'s switch in [Sources/McMacWindowCore/Geometry.swift](Sources/McMacWindowCore/Geometry.swift).
3. Add a binding in `HotkeyManager.bindings` in [Sources/McMacWindowCore/HotkeyManager.swift](Sources/McMacWindowCore/HotkeyManager.swift).
4. Add a unit test in [Tests/McMacWindowTests/GeometryTests.swift](Tests/McMacWindowTests/GeometryTests.swift) with exact expected values.
...
**No Xcode project, no external dependencies** — The project uses SPM but has zero external dependencies. Adding third-party packages or Xcode project files requires explicit approval.
